...
class TestRentContract(CreateRentalData):
    ...
    def test_write(self):
        ...
        message = "For Rent Unit '" + self.contract_one.rent_unit + "', Payment Term should be one of " + ', '.join(
            valid_payment_terms[self.contract_one.rent_unit])
    ...
    def test_action_button(self):
        ...
        self.assertEqual(action["params"]["message"],
                         "To close the contract, please settle all outstanding installment invoices for this contract.")
    ...
    def test_scheduler_autometic(self):
        ...
        self.env['ir.config_parameter'].set_param(
            'rental_management.reminder_days', '5')
        self.contract_six.last_invoice_payment_date = datetime.datetime.today().date() - relativedelta(
            years=1) + relativedelta(days=5)
