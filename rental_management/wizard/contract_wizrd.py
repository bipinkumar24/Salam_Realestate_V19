...
class ContractWizard(models.TransientModel):
    ...
    penalty_percentage = fields.Integer(string="Percentage ", default=lambda self: self.env[
                                                                                       'ir.config_parameter'].sudo().get_param(
    ...
    def default_get(self, fields_list):
        ...
        is_penalty_visible = self.env["ir.config_parameter"].sudo().get_param("rental_management.is_penalty_applied")
    ...
    @api.constrains('total_rent', 'deposit_amount', 'broker_commission', 'broker_commission_percentage')
    def _check_values_is_not_negative(self):
        ...
        for rec in self:
            ...
            if rec.deposit_amount and rec.deposit_amount < 0:
                raise ValidationError(_("Security deposit must be zero or greater"))
            if rec.broker_commission and rec.broker_commission < 0:
                raise ValidationError(_("Commission must be zero or greater"))
            if rec.broker_commission_percentage and rec.broker_commission_percentage < 0:
                raise ValidationError(_("Percentage must be zero or greater"))
    ...
    def _compute_available_payment_term(self):
        ...
        for rec in self:
            ...
            if rec.rent_unit == "Month":
                available_payment_term = "Available Payment Term  : Monthly, Quarterly, Half year, Full Payment"
            elif rec.rent_unit == "Year":
                available_payment_term = "Available Payment Term  : Yearly, Full Payment"
            elif rec.rent_unit == "Day":
                available_payment_term = "Available Payment Term  : Monthly, Quarterly, Half Year, Yearly, Daily, Full Payment"
        ...
        for rec in self:
            ...
            if rec.property_id and rec.is_extra_service:
                for data in rec.property_id.extra_service_ids:
                    services += (
                        f"{data.service_id.name}[{'Once' if data.service_type == 'once' else 'Recurring'}] "
                        f"- {rec.currency_id.symbol} {data.price}\n")
    ...
    def contract_action(self):
        ...
        if self.rent_unit == "Month" and self.payment_term in ["daily", "yearly"]:
            message = {
                ...
                "params": {
                    "type": "info",
                    "title": (_("For Rent unit Month, please select a payment"
                                " term: either 'Monthly', 'Quarterly', or 'Full Payment'.")),
                    "sticky": False,
                },
            }
            return message
